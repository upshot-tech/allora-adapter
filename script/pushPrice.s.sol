...
contract PushPrice is Script {
    function run() public virtual {
        ...
        bytes32 priceMessage = prices.getPriceMessage(
            pdws.feedId, pdws.nonce, pdws.timestamp, pdws.price, pdws.extraData
        );
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(privateKey, ECDSA.toEthSignedMessageHash(priceMessage));
        ...
        uint256 totalFee = prices.totalFee();
        // send the actual TX that pushes the price, paying 1 eth as fee
        uint256 price = prices.getPrice{value: totalFee}(pds, '');
